...
- **ผล backtest ของ config ที่ใช้จริงตอนนี้** (divergence เปิด, HTF ปิด) บนข้อมูลชุดเดียวกัน
  89,985 แท่ง: 171 ไม้, win rate 39.8%, **expectancy +0.767R**, profit factor 2.07,
  max drawdown −13.4R โดย in-sample (+0.717R) กับ out-of-sample (+0.909R) สอดคล้องกัน
  ซึ่งเป็นจุดสำคัญที่สุดที่ต้องเช็ค — ตัวเลขที่ดีเฉพาะ in-sample คือ overfitting ไม่ใช่ edge จริง
  ส่วน win rate ที่ต่ำกว่า 50% เป็นเรื่องปกติของกลยุทธ์ที่กรองด้วย RR (`min_rr: 1.5`)
  คือชนะไม่ถึงครึ่งแต่ยังกำไรได้
