...
def flex_signal(signal, disclaimer: str, size_text: str | None = None) -> dict:
    ...
    if grade and grade != "?":
        ...
        note = {"A": "โมเมนตัมหมดแรงชัด", "B": "อ่อนแรงเล็กน้อย"}.get(grade, "ยังมีแรงสวน")
        detail = f"  (RSI −{ls_diff:.0f})" if ls_diff and ls_diff > 0 else ""
        body.append(_row(f"เกรด {grade}", f"{note}{detail}", colour, bold=(grade == "A")))
