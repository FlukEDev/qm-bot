...
## สรุปเทรนด์ Bitcoin รายวัน

แยกจากตัวสแกนสัญญาณ: `daily_report.py` ส่งภาพรวม Bitcoin เข้า LINE
วันละครั้ง เวลา **10:00 น. ตามเวลาไทย** ไม่มีจุดเข้า/SL/TP เพราะไม่ใช่สัญญาณเทรด
แต่เป็นบริบทว่าตอนนี้ตลาดอยู่ตรงไหน

ทุกตัวเลขคำนวณจากราคาที่บอทดึงจาก Binance อยู่แล้ว ไม่ต้องใช้บริการภายนอก
ไม่ต้องมี API key เพิ่ม และไม่มีค่าใช้จ่ายเพิ่ม:

| ข้อมูล | ที่มา |
|---|---|
| ราคา, เปลี่ยนแปลง 24 ชม./7 วัน | ราคาปิด 1H |
| เทรนด์ 1D / 4H | โครงสร้าง swing จาก `htf_filter.htf_structure` |
| RSI 1D / 4H | `divergence.rsi` |
| ความผันผวน | ATR(14) เทียบเป็น % ของราคา และเทียบกับค่าเฉลี่ย 90 วันของตัวเอง |
| แนวรับ / แนวต้าน | swing pivot รายวันที่ยืนยันแล้ว ใกล้ราคาปัจจุบันที่สุด |

```bash
python daily_report.py --dry-run   # พิมพ์ออกจอ ไม่ส่งจริง
python daily_report.py             # ส่งเข้า LINE
python daily_report.py --force     # ส่งทันทีแม้ระบบจำกัดโควตาจะสั่งข้าม
```

**การจำกัดตามโควตา** สัญญาณเทรดสำคัญกว่าสรุปสถานะ ตัวสรุปจึงถอยให้เมื่อโควตา
LINE ของเดือนนั้นเริ่มร่อยหรอ วัดจริงบน config ปัจจุบันได้ ~171 สัญญาณ + 30 สรุป
= 201 จากโควตาฟรี 300 ข้อความ ถ้าเดือนไหนสัญญาณพุ่ง กลไกนี้จะกันโควตาที่เหลือ
ไว้ให้สัญญาณ:

- ใช้ไป **≤ 50%** → ส่งสรุปทุกวันตามปกติ
- ใช้ไป **> 50%** → ส่งเฉพาะวันจันทร์ และในข้อความจะบอกเหตุผลไว้ด้วย

การเช็คเกิดขึ้นทุกครั้งที่จะส่ง ไม่ใช่สัปดาห์ละครั้ง เพราะถ้าเช็คสัปดาห์ละครั้ง
โควตาอาจทะลุเกณฑ์ตั้งแต่วันอังคารแล้วยังส่งรายวันต่อจนถึงจันทร์หน้ากว่าจะรู้ตัว
ส่วนการเช็คทุกครั้งไม่มีต้นทุนเพิ่มและตอบสนองทันที อีกทั้งยังกลับมาส่งรายวันเอง
อัตโนมัติเมื่อ LINE รีเซ็ตตัวนับต้นเดือน โดยไม่ต้องเก็บสถานะไว้ให้เพี้ยน
ถ้าอ่านโควตาไม่ได้จะส่งตามปกติ เพราะการที่ระบบตรวจสอบล่มไม่ใช่เหตุผลที่จะเงียบหาย

ตั้งค่าได้ที่ `daily_report.quota_throttle` (`threshold_pct`, `weekly_day`,
`enabled`) และทุกสรุปจะมีแถว `โควตา LINE` บอกยอดใช้งาน เพื่อให้เห็นตัวเลข
ก่อนที่มันจะกลายเป็นปัญหา


ตั้งเวลาด้วย systemd timer (`deploy/qmbot-daily.timer`) ไม่ใช่ cron —
`OnCalendar=*-*-* 10:00:00 Asia/Bangkok` ระบุ timezone ตรงๆ เพื่อให้เวลายังถูก
แม้ timezone ของเครื่องจะถูกเปลี่ยน และ `Persistent=true` ทำให้ถ้า VM ปิดอยู่
ตอน 10 โมง พอเปิดมาจะส่งให้ทันที ไม่ข้ามวันไปเงียบๆ

```bash
# ดูว่าจะส่งครั้งถัดไปเมื่อไหร่
gcloud compute ssh qm-bot --zone=asia-southeast1-a --command="systemctl list-timers qmbot-daily.timer"
# สั่งส่งเดี๋ยวนี้เลย
gcloud compute ssh qm-bot --zone=asia-southeast1-a --command="sudo systemctl start qmbot-daily.service"
```
