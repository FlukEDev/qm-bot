...
## Daily Bitcoin trend report

Separate from the signal scanner: `daily_report.py` sends one Bitcoin
overview to LINE every day at **10:00 Thailand time**. It carries no entry,
stop or target — it is context (where the market stands), not a trade.

Every number comes from Binance price data the bot already pulls, so there
is no extra service, credential or cost involved:

| field | source |
|---|---|
| price, 24h / 7d change | 1H closes |
| trend 1D / 4H | swing structure via `htf_filter.htf_structure` |
| RSI 1D / 4H | `divergence.rsi` |
| volatility | ATR(14) as % of price, vs its own 90-day average |
| support / resistance | nearest confirmed daily swing pivots |

```bash
python daily_report.py --dry-run   # print, do not send
python daily_report.py             # send to LINE
python daily_report.py --force     # send even if the quota throttle says skip
```

**Quota throttle.** Trade signals matter more than a status summary, so the
report steps aside when the month's LINE allowance runs low. Measured usage
on the current config is ~171 signals + 30 reports = 201 of the 300-message
free tier; if signals ever surge, this keeps the remaining allowance for
them:

- monthly usage **≤ 50%** → report every day (normal)
- monthly usage **> 50%** → report only on Mondays, and the message says so

The check runs at each report rather than once a week: a weekly check would
let usage overshoot on a Tuesday and keep spending daily until the next
Monday noticed, whereas checking each time costs nothing and reacts at once.
It also returns to daily on its own when LINE's monthly counter resets — no
stored state to drift. If the quota endpoint cannot be reached the report is
sent anyway, since a monitoring failure is no reason to go quiet.

Configure under `daily_report.quota_throttle` (`threshold_pct`, `weekly_day`,
`enabled`). Every report carries a `โควตา LINE` row showing usage, so the
number is visible before it becomes a problem.


Scheduling is a systemd timer (`deploy/qmbot-daily.timer`), not cron:
`OnCalendar=*-*-* 10:00:00 Asia/Bangkok` names the zone explicitly so the
schedule survives a host timezone change, and `Persistent=true` means a
report missed while the VM was off is sent on next boot rather than skipped.

```bash
gcloud compute ssh qm-bot --zone=asia-southeast1-a --command="systemctl list-timers qmbot-daily.timer"
gcloud compute ssh qm-bot --zone=asia-southeast1-a --command="sudo systemctl start qmbot-daily.service"  # send now
```
...
- **Backtest coverage.** Two datasets are used, and they answer different
  questions:

  | dataset | symbols | span | what it is good for |
  |---|---|---|---|
  | 15 majors | 15 | 5.9–6.6 years each | regime robustness (2021 bull, 2022 bear, 2023-26) |
  | full universe | 99 | 1 month – 6.6 years | matches what the bot actually scans |

  The 99-symbol set is the live universe, so it carries **survivorship bias**:
  these are the coins with high volume *today*. Ones that died or fell out of
  the top 100 are absent, and many entries are recent listings with only
  months of history — which tilts the aggregate toward the 2025-26 regime.
  Treat the 15-major set as the check on whether a result survives across
  market regimes, and the 99-symbol set as the check on live behaviour.

  Latest run on the full universe (2.6M 1H bars, per-profile gates):

  | | in-sample | out-of-sample |
  |---|---|---|
  | 1h short | n=3027, +0.592R, PF 1.94 | n=1428, +0.634R, PF 2.04 |
  | 1h wide | n=416, +0.290R, PF 1.41 | n=150, +0.631R, PF 1.93 |
  | combined | n=3443, +0.555R | n=1578, +0.634R |
