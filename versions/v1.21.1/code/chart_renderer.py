...
def render_signal(
    ...
    # Endpoints come from the anchored indices computed in divergence.py, not
    # from the price pivots. A divergence line joins peaks of the oscillator
    # itself; drawing to RSI-at-the-price-pivot put the ends on the slope and
    # sent the line straight through the RSI curve on ~3 signals in 4.
    i_bos = getattr(signal, "i_bos_rsi", signal.l2_idx)
    i_rs = getattr(signal, "i_rs_rsi", signal.trigger_idx)
    r_bos = float(rsi_series.iloc[i_bos]) if i_bos < len(rsi_series) else None
    r_trig0 = float(rsi_series.iloc[i_rs]) if i_rs < len(rsi_series) else None
    bos_rs = getattr(signal, "bos_rs", None)
    if r_bos is not None and r_trig0 is not None and pd.notna(r_bos) and pd.notna(r_trig0):
        # Markers, not a connecting line. This pair is a trough and a peak —
        # different extremum types — so a straight line between them is not a
        # divergence line and the RSI curve necessarily wanders across it.
        # Drawing it as a line implied a comparison that does not exist; the
        # number it measures (how far RSI travelled on the bounce) is shown in
        # the label instead.
        ax_rsi.plot(
            [idx[i_bos], idx[i_rs]],
            [r_bos, r_trig0],
        ...
        # a light vertical tick at each end makes the two levels readable
        # without implying a path between them
        for xi, yi in ((idx[i_bos], r_bos), (idx[i_rs], r_trig0)):
            ax_rsi.plot([xi, xi], [yi - 3, yi + 3], color=DIV_COLOR,
    ...
    i_head = getattr(signal, "i_head_rsi", signal.head_idx)
    i_trig = getattr(signal, "i_trig_rsi", signal.trigger_idx)
    r_head = float(rsi_series.iloc[i_head]) if i_head < len(rsi_series) else None
    r_trig = float(rsi_series.iloc[i_trig]) if i_trig < len(rsi_series) else None
