...
class LineNotifier:
    ...
    def quota_status(self) -> dict | None:
        """How much of this month's LINE message allowance is spent.

        Returns {"used", "limit", "ratio"} — or None when the plan has no
        limit, or when LINE cannot be reached. Callers should treat None as
        "no information" and carry on rather than blocking themselves: a
        monitoring lookup failing is not a reason to stop delivering.
        """
        headers = {"Authorization": f"Bearer {self.token}"}
        try:
            q = requests.get(f"{API_BASE}/message/quota", headers=headers, timeout=TIMEOUT)
            c = requests.get(f"{API_BASE}/message/quota/consumption",
                             headers=headers, timeout=TIMEOUT)
            if q.status_code != 200 or c.status_code != 200:
                return None
            quota, cons = q.json(), c.json()
        except requests.RequestException:
            return None

        if quota.get("type") != "limited":
            return None  # unlimited plan — nothing to ration
        limit = int(quota.get("value", 0))
        used = int(cons.get("totalUsage", 0))
        if limit <= 0:
            return None
        return {"used": used, "limit": limit, "ratio": used / limit}
...
def flex_signal(signal, disclaimer: str, size_text: str | None = None) -> dict:
    ...
    bos_body = getattr(signal, "bos_body_close", None)
    if bos_body is not None:
        body.append(_row("BOS", "เนื้อปิดทะลุ ✓" if bos_body else "ไส้แทะเท่านั้น",
                          "#26A69A" if bos_body else "#8C8C8C", bold=bos_body))
