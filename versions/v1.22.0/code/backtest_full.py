...
from candle_quality import passes_candle_gate, tag_candle_quality
...
def run_pipeline(
    ...
    candle_gate: bool = True,
    require_bos_body: bool = True,
    require_head_wick: bool = False,
) -> list:
    ...
    signals = attach_divergence(
        ...
        bos_rs_max=bos_rs_max,
        ls_min_diff=ls_min_diff,
    )
    # Candle-quality gate — same position in the chain as bot.scan_once():
    # after divergence, before the HTF filter. Leaving it out here made this
    # module measure a strategy the live bot no longer trades.
    signals = tag_candle_quality(signals, df_ltf)
    if candle_gate:
        signals = [s for s in signals
                   if passes_candle_gate(s, require_bos_body, require_head_wick)]
...
def main() -> None:
    ...
    ap.add_argument("--data-dir", default=None,
                    help="load <SYM>_1h.csv from this directory instead of the API "
                         "(see fetch_binance_vision.py). 4H is resampled from 1H.")
    ap.add_argument("--profile", default=None,
                    help="use a named profile from config.yaml's qm.'1h' list, "
                         "e.g. --profile wide")
    args = ap.parse_args()
    ...
    if args.data_dir:
        # Local CSVs from data.binance.vision — years of history instead of the
        # ~1000 bars a single API call returns. 4H is resampled from 1H rather
        # than downloaded: Binance 4H bars open at 00/04/08/12/16/20 UTC, which
        # is exactly what a UTC-anchored 4h resample produces.
        ddir = Path(args.data_dir)
        print(f"loading local history from {ddir}/ ...")
        for sym in symbols:
            stem = display_symbol(sym).replace("/", "")
            path = ddir / f"{stem}_1h.csv"
            if not path.exists():
                print(f"  {stem:<14} MISSING {path}")
                continue
            d1 = pd.read_csv(path, parse_dates=["time"]).set_index("time")
            d1 = d1[["open", "high", "low", "close"]].astype(float)
            d4 = (d1.resample("4h")
                    .agg({"open": "first", "high": "max", "low": "min", "close": "last"})
                    .dropna())
            data[sym] = {"1h": d1, "4h": d4}
            print(f"  {stem:<14} 1h={len(d1):<7} 4h={len(d4):<6} "
    ...
    if args.profile:
        # Pull the real live settings out of config.yaml so the backtest tests
        # what the bot actually runs, not a hand-typed approximation of it.
        import yaml

        from bot import profiles_for

        qm_cfg = yaml.safe_load(Path("config.yaml").read_text())["qm"]
        found = {name.split(" ", 1)[-1]: kw for name, kw in profiles_for(qm_cfg, "1h")}
        if args.profile not in found:
            raise SystemExit(f"no profile {args.profile!r}; have {sorted(found)}")
        base_qm = found[args.profile]
        print(f"\nprofile '{args.profile}' from config.yaml: "
              f"pivot={base_qm['pivot_left']} retest={base_qm['max_bars_to_retest']} "
              f"min_rr={base_qm['min_rr']}")
