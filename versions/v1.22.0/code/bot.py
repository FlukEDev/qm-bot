    ...
    for symbol in symbols:  # ccxt-ready perpetual futures symbol, e.g. "BTC/USDT:USDT"
        ...
        for tf in timeframes:
            ...
            for label, qm_kwargs in profiles_for(qm_cfg_by_tf, tf):
                ...
                ls_cfg = div_cfg.get("last_swing", {}) or {}
                signals = attach_divergence(
                    signals,
                    df,
                    rsi_period=div_cfg.get("rsi_period", 14),
                    min_rsi_diff=div_cfg.get("min_rsi_diff", 2.0),
                    required=div_cfg.get("required", False),
                    ls_grade_a=ls_cfg.get("grade_a_min", 5.0),
                    ls_grade_b=ls_cfg.get("grade_b_min", 0.0),
                    ls_min_diff=ls_cfg.get("min_diff"),
                    # Per-profile override: swing scale changes what a
                    # "weak bounce" looks like in RSI points.
                    bos_rs_max=(div_cfg.get("bos_rs_max_by_profile", {}) or {}).get(
                        label, div_cfg.get("bos_rs_max", 10.0)),
                )

                cq = cfg.get("candle_quality", {}) or {}
                signals = tag_candle_quality(signals, df)
                if cq.get("enabled", True):
                    before_cq = len(signals)
                    signals = [s for s in signals if passes_candle_gate(
                        s, cq.get("require_bos_body", True), cq.get("require_head_wick", False))]
                    if before_cq - len(signals):
                        log.info("candle gate dropped %d/%d %s %s signal(s)",
                                 before_cq - len(signals), before_cq, disp, label)
                ...
                for s in signals:
                    ...
                    log.info(
                        "SIGNAL %s %s %s entry=%.4f RR=%.2f span=%db grade=%s(%.1f) bosBody=%s htf=%s",
                        disp, label, s.direction, s.entry, s.risk_reward,
                        s.trigger_idx - s.ls_idx,
                        getattr(s, "ls_grade", "?"),
                        getattr(s, "ls_diff", None) or 0.0,
                        getattr(s, "bos_body_close", None),
