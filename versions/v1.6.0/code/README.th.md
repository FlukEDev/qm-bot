...
- **ตัวกรอง HTF (แนวโน้ม timeframe ใหญ่กว่า)** (`htf_filter` ใน `config.yaml`
  เปิดใช้งานอยู่โดย default): จะตัดสัญญาณ 1H ที่สวนทางกับแนวโน้ม 4H อย่างชัดเจนทิ้ง
  (เช่น สัญญาณ bearish ตอนที่ 4H กำลังทำ higher-high/higher-low หรือกลับด้าน) นี่คือ
  ตัวกรอง confluence ที่มีคุณค่าสูงสุดตามเอกสารทฤษฎีของ skill เองและงานวิจัยที่รองรับ —
  ดูที่ `htf_filter.py` ตัวกรองนี้มีหน้าที่ตัดสัญญาณออกเท่านั้น ไม่เพิ่มสัญญาณที่ผ่านเกณฑ์
  RR/divergence ไม่ได้อยู่แล้ว ดังนั้นคาดว่าจำนวนสัญญาณจะ**น้อยลงกว่าเดิมอย่างเห็นได้ชัด**
  — นั่นคือ tradeoff ที่ตั้งใจ (สัญญาณน้อยลงแต่คุณภาพสูงขึ้น) ปิดได้ด้วย
  `htf_filter.enabled: false` แล้วเทียบผลด้วย `backtest.py --split`
