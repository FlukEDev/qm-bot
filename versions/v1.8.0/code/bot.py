...
def scan_once(cfg: dict, exchange, store: SignalStore, notifier, dry_run: bool = False) -> int:
    ...
    for symbol in symbols:  # ccxt-ready perpetual futures symbol, e.g. "BTC/USDT:USDT"
        ...
        for tf in timeframes:
            ...
            qm_cfg = QMConfig(**qm_cfg_by_tf.get(tf, {}))
            signals = detect_qm(df, qm_cfg, symbol=disp, timeframe=tf)
            ...
            if htf_enabled and tf in htf_reference:
                ...
                if ref_df is not None:
                    ...
                    before = len(signals)
                    for s in signals:
                        s.htf_structure = structure  # tag every signal for display, even if dropped
                    ...
                    if dropped:
                        log.info(
                            ...
                            ref_tf, structure, dropped, before, disp, tf,
                        )
            ...
            for s in signals:
                ...
                log.info(
                    "SIGNAL %s %s %s entry=%.4f RR=%.2f divergence=%s htf=%s",
                    disp, tf, s.direction, s.entry, s.risk_reward,
                ...
                if dry_run:
                    ...
                    png = render_signal(
                        df, s, out_path=f"/tmp/{s.signal_id.replace('|', '_').replace('/', '-')}.png"
                    )
                    chart_url = upload_chart(png)
                    notifier.send_signal(s, to=line_to, chart_url=chart_url, size_text=size_text)
