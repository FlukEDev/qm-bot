...
def render_signal(
    ...
    div_tag = " · Divergence ✓" if confirmed else (" · Divergence ✗" if confirmed is False else "")
    ax.set_title(
        f"{signal.symbol} {signal.timeframe} — QM {arrow}{tag}{div_tag}  ·  RR {signal.risk_reward}",
        color=FG,
        fontsize=13,
        fontweight="bold",
    )
    ...
    r_ls = float(rsi_series.iloc[signal.ls_idx]) if signal.ls_idx < len(rsi_series) else None
    r_head = float(rsi_series.iloc[signal.head_idx]) if signal.head_idx < len(rsi_series) else None
    if r_ls is not None and r_head is not None and pd.notna(r_ls) and pd.notna(r_head):
        div_label = "Bearish Div" if signal.direction == "bearish" else "Bullish Div"
        line_style = "-" if confirmed else "--"
        ax_rsi.plot(
            [idx[signal.ls_idx], idx[signal.head_idx]],
            [r_ls, r_head],
            color=DIV_COLOR,
            linewidth=1.6,
        ...
        ax_rsi.annotate(
            f"{div_label}{'' if confirmed else ' (not confirmed)'}",
            (idx[signal.head_idx], r_head),
            textcoords="offset points",
            xytext=(6, 8 if signal.direction == "bullish" else -14),
            color=DIV_COLOR,
            fontsize=8,
            fontweight="bold",
        )
